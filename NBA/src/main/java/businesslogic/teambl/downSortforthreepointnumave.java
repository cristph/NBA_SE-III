...
public class downSortforthreepointnumave {
     ...
     */
      ...
      */
      public static TeamVO median( ArrayList<TeamVO> arr, int left, int right ) {
		...
		if ( arr.get(left).getThreePointNumave() <arr.get(center).getThreePointNumave()  )
			swapRef( arr, left, center );
		if ( arr.get(left).getThreePointNumave() <arr.get(right).getThreePointNumave() )
			swapRef( arr, left, right );
		if ( arr.get(center).getThreePointNumave() <arr.get(right).getThreePointNumave() )
      ...
      */
      private static  void quickSort( ArrayList<TeamVO> arr, int left, int right ) {
		if ( left + CUTOFF <= right  ) {
			...
			for ( ; ; ) {
				while ( arr.get(++i).getThreePointNumave()>pivot.getThreePointNumave()) ;
				while (arr.get(--j).getThreePointNumave()<pivot.getThreePointNumave() ) ;
				if ( i < j )
					swapRef( arr, i, j );
				else
					break;
      ...
      public static  void insertionSort( ArrayList<TeamVO> arr, int start, int end ) {
		...
		for ( int j = start + 1; j <= end; j++ ) {
			...
			for ( i = j; i > start && tmp.getThreePointNumave()> arr.get(i - 1).getThreePointNumave() ; i-- ) {
