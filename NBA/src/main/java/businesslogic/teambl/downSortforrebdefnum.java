...
public class downSortforrebdefnum {
     ...
     */
      ...
      */
      public static TeamVO median( ArrayList<TeamVO> arr, int left, int right ) {
		...
		if ( arr.get(left).getRebDefNum() <arr.get(center).getRebDefNum()  )
			swapRef( arr, left, center );
		if ( arr.get(left).getRebDefNum() <arr.get(right).getRebDefNum())
			swapRef( arr, left, right );
		if ( arr.get(center).getRebDefNum() <arr.get(right).getRebDefNum() )
      ...
      */
      private static  void quickSort( ArrayList<TeamVO> arr, int left, int right ) {
		if ( left + CUTOFF <= right  ) {
			...
			for ( ; ; ) {
				while ( arr.get(++i).getRebDefNum()>pivot.getRebDefNum()) ;
				while (arr.get(--j).getRebDefNum()<pivot.getRebDefNum() ) ;
				if ( i < j )
					swapRef( arr, i, j );
				else
					break;
      ...
      public static  void insertionSort( ArrayList<TeamVO> arr, int start, int end ) {
		...
		for ( int j = start + 1; j <= end; j++ ) {
			...
			for ( i = j; i > start && tmp.getRebDefNum()> arr.get(i - 1).getRebDefNum() ; i-- ) {
