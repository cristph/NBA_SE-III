...
public class downSortforshootnumave {
     ...
     */
      ...
      */
      public static TeamVO median( ArrayList<TeamVO> arr, int left, int right ) {
		...
		if ( arr.get(left).getShootNumave()<arr.get(center).getShootNumave() )
			swapRef( arr, left, center );
		if ( arr.get(left).getShootNumave() <arr.get(right).getShootNumave() )
			swapRef( arr, left, right );
		if ( arr.get(center).getShootNumave() <arr.get(right).getShootNumave())
      ...
      */
      private static  void quickSort( ArrayList<TeamVO> arr, int left, int right ) {
		if ( left + CUTOFF <= right  ) {
			...
			for ( ; ; ) {
				while ( arr.get(++i).getShootNumave()>pivot.getShootNumave() ) ;
				while (arr.get(--j).getShootNumave()<pivot.getShootNumave() ) ;
				if ( i < j )
					swapRef( arr, i, j );
				else
					break;
      ...
      public static  void insertionSort( ArrayList<TeamVO> arr, int start, int end ) {
		...
		for ( int j = start + 1; j <= end; j++ ) {
			...
			for ( i = j; i > start && tmp.getShootNumave()> arr.get(i - 1).getShootNumave() ; i-- ) {
