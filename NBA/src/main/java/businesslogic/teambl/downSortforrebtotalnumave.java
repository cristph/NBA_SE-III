...
public class downSortforrebtotalnumave {
     ...
     */
      ...
      */
      public static TeamVO median( ArrayList<TeamVO> arr, int left, int right ) {
		...
		if ( arr.get(left).getRebTotalNumave() <arr.get(center).getRebTotalNumave()  )
			swapRef( arr, left, center );
		if ( arr.get(left).getRebTotalNumave()<arr.get(right).getRebTotalNumave() )
			swapRef( arr, left, right );
		if ( arr.get(center).getRebTotalNumave()<arr.get(right).getRebTotalNumave() )
      ...
      */
      private static  void quickSort( ArrayList<TeamVO> arr, int left, int right ) {
		if ( left + CUTOFF <= right  ) {
			...
			for ( ; ; ) {
				while ( arr.get(++i).getRebTotalNumave()>pivot.getRebTotalNumave() ) ;
				while (arr.get(--j).getRebTotalNumave()<pivot.getRebTotalNumave() ) ;
				if ( i < j )
					swapRef( arr, i, j );
				else
					break;
      ...
      public static  void insertionSort( ArrayList<TeamVO> arr, int start, int end ) {
		...
		for ( int j = start + 1; j <= end; j++ ) {
			...
			for ( i = j; i > start && tmp.getRebTotalNumave()> arr.get(i - 1).getRebTotalNumave() ; i-- ) {
