...
public class downSortforfoulnumave {
     ...
     */
      ...
      */
      public static TeamVO median( ArrayList<TeamVO> arr, int left, int right ) {
		...
		if ( arr.get(left).getFoulNumave() <arr.get(center).getFoulNumave()  )
			swapRef( arr, left, center );
		if ( arr.get(left).getFoulNumave() <arr.get(right).getFoulNumave() )
			swapRef( arr, left, right );
		if ( arr.get(center).getFoulNumave() <arr.get(right).getFoulNumave())
      ...
      */
      private static  void quickSort( ArrayList<TeamVO> arr, int left, int right ) {
		if ( left + CUTOFF <= right  ) {
			...
			for ( ; ; ) {
				while ( arr.get(++i).getFoulNumave()>pivot.getFoulNumave()) ;
				while (arr.get(--j).getFoulNumave()<pivot.getFoulNumave() ) ;
				if ( i < j )
					swapRef( arr, i, j );
				else
					break;
      ...
      public static  void insertionSort( ArrayList<TeamVO> arr, int start, int end ) {
		...
		for ( int j = start + 1; j <= end; j++ ) {
			...
			for ( i = j; i > start && tmp.getFoulNumave()> arr.get(i - 1).getFoulNumave() ; i-- ) {
