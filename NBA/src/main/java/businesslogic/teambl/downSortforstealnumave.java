...
public class downSortforstealnumave {
     ...
     */
      ...
      */
      public static TeamVO median( ArrayList<TeamVO> arr, int left, int right ) {
		...
		if ( arr.get(left).getStealNumave() <arr.get(center).getStealNumave() )
			swapRef( arr, left, center );
		if ( arr.get(left).getStealNumave() <arr.get(right).getStealNumave())
			swapRef( arr, left, right );
		if ( arr.get(center).getStealNumave() <arr.get(right).getStealNumave() )
      ...
      */
      private static  void quickSort( ArrayList<TeamVO> arr, int left, int right ) {
		if ( left + CUTOFF <= right  ) {
			...
			for ( ; ; ) {
				while ( arr.get(++i).getStealNumave()>pivot.getStealNumave() ) ;
				while (arr.get(--j).getStealNumave()<pivot.getStealNumave() ) ;
				if ( i < j )
					swapRef( arr, i, j );
				else
					break;
      ...
      public static  void insertionSort( ArrayList<TeamVO> arr, int start, int end ) {
		...
		for ( int j = start + 1; j <= end; j++ ) {
			...
			for ( i = j; i > start && tmp.getStealNumave()> arr.get(i - 1).getStealNumave() ; i-- ) {
