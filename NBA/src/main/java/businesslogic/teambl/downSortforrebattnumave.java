...
public class downSortforrebattnumave {
     ...
     */
      ...
      */
      public static TeamVO median( ArrayList<TeamVO> arr, int left, int right ) {
		...
		if ( arr.get(left).getRebAttNumave() <arr.get(center).getRebAttNumave() )
			swapRef( arr, left, center );
		if ( arr.get(left).getRebAttNumave() <arr.get(right).getRebAttNumave() )
			swapRef( arr, left, right );
		if ( arr.get(center).getRebAttNumave() <arr.get(right).getRebAttNumave() )
      ...
      */
      private static  void quickSort( ArrayList<TeamVO> arr, int left, int right ) {
		if ( left + CUTOFF <= right  ) {
			...
			for ( ; ; ) {
				while ( arr.get(++i).getRebAttNumave()>pivot.getRebAttNumave() ) ;
				while (arr.get(--j).getRebAttNumave()<pivot.getRebAttNumave() ) ;
				if ( i < j )
					swapRef( arr, i, j );
				else
					break;
      ...
      public static  void insertionSort( ArrayList<TeamVO> arr, int start, int end ) {
		...
		for ( int j = start + 1; j <= end; j++ ) {
			...
			for ( i = j; i > start && tmp.getRebAttNumave()> arr.get(i - 1).getRebAttNumave() ; i-- ) {
