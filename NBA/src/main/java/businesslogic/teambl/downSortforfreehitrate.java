...
public class downSortforfreehitrate {
     ...
     */
      ...
      */
      public static TeamVO median( ArrayList<TeamVO> arr, int left, int right ) {
		...
		if ( arr.get(left).getFreehitrate() <arr.get(center).getFreehitrate()  )
			swapRef( arr, left, center );
		if ( arr.get(left).getFreehitrate() <arr.get(right).getFreehitrate() )
			swapRef( arr, left, right );
		if ( arr.get(center).getFreehitrate() <arr.get(right).getFreehitrate() )
      ...
      */
      private static  void quickSort( ArrayList<TeamVO> arr, int left, int right ) {
		if ( left + CUTOFF <= right  ) {
			...
			for ( ; ; ) {
				while ( arr.get(++i).getFreehitrate()>pivot.getFreehitrate() ) ;
				while (arr.get(--j).getFreehitrate()<pivot.getFreehitrate()) ;
				if ( i < j )
					swapRef( arr, i, j );
				else
					break;
      ...
      public static  void insertionSort( ArrayList<TeamVO> arr, int start, int end ) {
		...
		for ( int j = start + 1; j <= end; j++ ) {
			...
			for ( i = j; i > start && tmp.getFreehitrate()> arr.get(i - 1).getFreehitrate() ; i-- ) {
