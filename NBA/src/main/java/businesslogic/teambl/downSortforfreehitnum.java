...
public class downSortforfreehitnum {
     ...
     */
      ...
      */
      public static TeamVO median( ArrayList<TeamVO> arr, int left, int right ) {
		...
		if ( arr.get(left).getFreeHitNum() <arr.get(center).getFreeHitNum()  )
			swapRef( arr, left, center );
		if ( arr.get(left).getFreeHitNum() <arr.get(right).getFreeHitNum() )
			swapRef( arr, left, right );
		if ( arr.get(center).getFreeHitNum() <arr.get(right).getFreeHitNum() )
      ...
      */
      private static  void quickSort( ArrayList<TeamVO> arr, int left, int right ) {
		if ( left + CUTOFF <= right  ) {
			...
			for ( ; ; ) {
				while ( arr.get(++i).getFreeHitNum()>pivot.getFreeHitNum() ) ;
				while (arr.get(--j).getFreeHitNum()<pivot.getFreeHitNum() ) ;
				if ( i < j )
					swapRef( arr, i, j );
				else
					break;
      ...
      public static  void insertionSort( ArrayList<TeamVO> arr, int start, int end ) {
		...
		for ( int j = start + 1; j <= end; j++ ) {
			...
			for ( i = j; i > start && tmp.getFreeHitNum()> arr.get(i - 1).getFreeHitNum() ; i-- ) {
