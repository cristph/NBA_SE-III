...
public class downSortfordefrate {
     ...
     */
      ...
      */
      public static TeamVO median( ArrayList<TeamVO> arr, int left, int right ) {
		...
		if ( arr.get(left).getDefendrate() <arr.get(center).getDefendrate()  )
			swapRef( arr, left, center );
		if ( arr.get(left).getDefendrate() <arr.get(right).getDefendrate() )
			swapRef( arr, left, right );
		if ( arr.get(center).getDefendrate() <arr.get(right).getDefendrate())
      ...
      */
      private static  void quickSort( ArrayList<TeamVO> arr, int left, int right ) {
		if ( left + CUTOFF <= right  ) {
			...
			for ( ; ; ) {
				while ( arr.get(++i).getDefendrate()>pivot.getDefendrate() ) ;
				while (arr.get(--j).getDefendrate()<pivot.getDefendrate() ) ;
				if ( i < j )
					swapRef( arr, i, j );
				else
					break;
      ...
      public static  void insertionSort( ArrayList<TeamVO> arr, int start, int end ) {
		...
		for ( int j = start + 1; j <= end; j++ ) {
			...
			for ( i = j; i > start && tmp.getDefendrate()> arr.get(i - 1).getDefendrate() ; i-- ) {
