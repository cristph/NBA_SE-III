...
public class downSortforrebdefnumave {
     ...
     */
      ...
      */
      public static TeamVO median( ArrayList<TeamVO> arr, int left, int right ) {
		...
		if ( arr.get(left).getRebDefNumave() <arr.get(center).getRebDefNumave() )
			swapRef( arr, left, center );
		if ( arr.get(left).getRebDefNumave()<arr.get(right).getRebDefNumave() )
			swapRef( arr, left, right );
		if ( arr.get(center).getRebDefNumave() <arr.get(right).getRebDefNumave() )
      ...
      */
      private static  void quickSort( ArrayList<TeamVO> arr, int left, int right ) {
		if ( left + CUTOFF <= right  ) {
			...
			for ( ; ; ) {
				while ( arr.get(++i).getRebDefNumave()>pivot.getRebDefNumave() ) ;
				while (arr.get(--j).getRebDefNumave()<pivot.getRebDefNumave()) ;
				if ( i < j )
					swapRef( arr, i, j );
				else
					break;
      ...
      public static  void insertionSort( ArrayList<TeamVO> arr, int start, int end ) {
		...
		for ( int j = start + 1; j <= end; j++ ) {
			...
			for ( i = j; i > start && tmp.getRebDefNumave()> arr.get(i - 1).getRebDefNumave() ; i-- ) {
