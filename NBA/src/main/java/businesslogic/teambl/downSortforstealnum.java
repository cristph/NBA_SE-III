...
public class downSortforstealnum {
     ...
     */
      ...
      */
      public static TeamVO median( ArrayList<TeamVO> arr, int left, int right ) {
		...
		if ( arr.get(left).getStealNum()<arr.get(center).getStealNum()  )
			swapRef( arr, left, center );
		if ( arr.get(left).getStealNum() <arr.get(right).getStealNum() )
			swapRef( arr, left, right );
		if ( arr.get(center).getStealNum() <arr.get(right).getStealNum() )
      ...
      */
      private static  void quickSort( ArrayList<TeamVO> arr, int left, int right ) {
		if ( left + CUTOFF <= right  ) {
			...
			for ( ; ; ) {
				while ( arr.get(++i).getStealNum()>pivot.getStealNum() ) ;
				while (arr.get(--j).getStealNum()<pivot.getStealNum() ) ;
				if ( i < j )
					swapRef( arr, i, j );
				else
					break;
      ...
      public static  void insertionSort( ArrayList<TeamVO> arr, int start, int end ) {
		...
		for ( int j = start + 1; j <= end; j++ ) {
			...
			for ( i = j; i > start && tmp.getStealNum()> arr.get(i - 1).getStealNum() ; i-- ) {
