...
public class downSortforattackround {
     ...
     */
      ...
      */
      public static TeamVO median( ArrayList<TeamVO> arr, int left, int right ) {
		...
		if ( arr.get(left).getAttackround() <arr.get(center).getAttackround() )
			swapRef( arr, left, center );
		if ( arr.get(left).getAttackround() <arr.get(right).getAttackround() )
			swapRef( arr, left, right );
		if ( arr.get(center).getAttackround()<arr.get(right).getAttackround() )
      ...
      */
      private static  void quickSort( ArrayList<TeamVO> arr, int left, int right ) {
		if ( left + CUTOFF <= right  ) {
			...
			for ( ; ; ) {
				while ( arr.get(++i).getAttackround()>pivot.getAttackround() ) ;
				while (arr.get(--j).getAttackround()<pivot.getAttackround() ) ;
				if ( i < j )
					swapRef( arr, i, j );
				else
					break;
      ...
      public static  void insertionSort( ArrayList<TeamVO> arr, int start, int end ) {
		...
		for ( int j = start + 1; j <= end; j++ ) {
			...
			for ( i = j; i > start && tmp.getAttackround()> arr.get(i - 1).getAttackround() ; i-- ) {
