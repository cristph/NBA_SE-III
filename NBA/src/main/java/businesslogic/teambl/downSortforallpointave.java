...
public class downSortforallpointave {
     ...
     */
      ...
      */
      public static TeamVO median( ArrayList<TeamVO> arr, int left, int right ) {
		...
		if ( arr.get(left).getAllpointave() <arr.get(center).getAllpointave()  )
			swapRef( arr, left, center );
		if ( arr.get(left).getAllpointave() <arr.get(right).getAllpointave() )
			swapRef( arr, left, right );
		if ( arr.get(center).getAllpointave() <arr.get(right).getAllpointave() )
      ...
      */
      private static  void quickSort( ArrayList<TeamVO> arr, int left, int right ) {
		if ( left + CUTOFF <= right  ) {
			...
			for ( ; ; ) {
				while ( arr.get(++i).getAllpointave()>pivot.getAllpointave() ) ;
				while (arr.get(--j).getAllpointave()<pivot.getAllpointave() ) ;
				if ( i < j )
					swapRef( arr, i, j );
				else
					break;
      ...
      public static  void insertionSort( ArrayList<TeamVO> arr, int start, int end ) {
		...
		for ( int j = start + 1; j <= end; j++ ) {
			...
			for ( i = j; i > start && tmp.getAllpointave()> arr.get(i - 1).getAllpointave() ; i-- ) {
