...
public class downSortforrebdefrate {
     ...
     */
      ...
      */
      public static TeamVO median( ArrayList<TeamVO> arr, int left, int right ) {
		...
		if ( arr.get(left).getRebdefrate()<arr.get(center).getRebdefrate()  )
			swapRef( arr, left, center );
		if ( arr.get(left).getRebdefrate() <arr.get(right).getRebdefrate() )
			swapRef( arr, left, right );
		if ( arr.get(center).getRebdefrate() <arr.get(right).getRebdefrate() )
      ...
      */
      private static  void quickSort( ArrayList<TeamVO> arr, int left, int right ) {
		if ( left + CUTOFF <= right  ) {
			...
			for ( ; ; ) {
				while ( arr.get(++i).getRebdefrate()>pivot.getRebdefrate() ) ;
				while (arr.get(--j).getRebdefrate()<pivot.getRebdefrate() ) ;
				if ( i < j )
					swapRef( arr, i, j );
				else
					break;
      ...
      public static  void insertionSort( ArrayList<TeamVO> arr, int start, int end ) {
		...
		for ( int j = start + 1; j <= end; j++ ) {
			...
			for ( i = j; i > start && tmp.getRebdefrate()> arr.get(i - 1).getRebdefrate() ; i-- ) {
