...
public class downSortforthreeshootnumave {
     ...
     */
      ...
      */
      public static TeamVO median( ArrayList<TeamVO> arr, int left, int right ) {
		...
		if ( arr.get(left).getThreeShootNumave()<arr.get(center).getThreeShootNumave()  )
			swapRef( arr, left, center );
		if ( arr.get(left).getThreeShootNumave() <arr.get(right).getThreeShootNumave())
			swapRef( arr, left, right );
		if ( arr.get(center).getThreeShootNumave()<arr.get(right).getThreeShootNumave())
      ...
      */
      private static  void quickSort( ArrayList<TeamVO> arr, int left, int right ) {
		if ( left + CUTOFF <= right  ) {
			...
			for ( ; ; ) {
				while ( arr.get(++i).getThreeShootNumave()>pivot.getThreeShootNumave() ) ;
				while (arr.get(--j).getThreeShootNumave()<pivot.getThreeShootNumave() ) ;
				if ( i < j )
					swapRef( arr, i, j );
				else
					break;
      ...
      public static  void insertionSort( ArrayList<TeamVO> arr, int start, int end ) {
		...
		for ( int j = start + 1; j <= end; j++ ) {
			...
			for ( i = j; i > start && tmp.getThreeShootNumave()> arr.get(i - 1).getThreeShootNumave() ; i-- ) {
