...
public class downSortforshootnum {
     ...
     */
      ...
      */
      public static TeamVO median( ArrayList<TeamVO> arr, int left, int right ) {
		...
		if ( arr.get(left).getShootNum() <arr.get(center).getShootNum()  )
			swapRef( arr, left, center );
		if ( arr.get(left).getShootNum() <arr.get(right).getShootNum() )
			swapRef( arr, left, right );
		if ( arr.get(center).getShootNum() <arr.get(right).getShootNum())
      ...
      */
      private static  void quickSort( ArrayList<TeamVO> arr, int left, int right ) {
		if ( left + CUTOFF <= right  ) {
			...
			for ( ; ; ) {
				while ( arr.get(++i).getShootNum()>pivot.getShootNum() ) ;
				while (arr.get(--j).getShootNum()<pivot.getShootNum() ) ;
				if ( i < j )
					swapRef( arr, i, j );
				else
					break;
      ...
      public static  void insertionSort( ArrayList<TeamVO> arr, int start, int end ) {
		...
		for ( int j = start + 1; j <= end; j++ ) {
			...
			for ( i = j; i > start && tmp.getShootNum()> arr.get(i - 1).getShootNum() ; i-- ) {
