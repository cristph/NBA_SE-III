...
public class downSortforstealrate {
     ...
     */
      ...
      */
      public static TeamVO median( ArrayList<TeamVO> arr, int left, int right ) {
		...
		if ( arr.get(left).getStealrate()<arr.get(center).getStealrate()  )
			swapRef( arr, left, center );
		if ( arr.get(left).getStealrate() <arr.get(right).getStealrate())
			swapRef( arr, left, right );
		if ( arr.get(center).getStealrate()<arr.get(right).getStealrate() )
      ...
      */
      private static  void quickSort( ArrayList<TeamVO> arr, int left, int right ) {
		if ( left + CUTOFF <= right  ) {
			...
			for ( ; ; ) {
				while ( arr.get(++i).getStealrate()>pivot.getStealrate()) ;
				while (arr.get(--j).getStealrate()<pivot.getStealrate() ) ;
				if ( i < j )
					swapRef( arr, i, j );
				else
					break;
      ...
      public static  void insertionSort( ArrayList<TeamVO> arr, int start, int end ) {
		...
		for ( int j = start + 1; j <= end; j++ ) {
			...
			for ( i = j; i > start && tmp.getStealrate()> arr.get(i - 1).getStealrate() ; i-- ) {
