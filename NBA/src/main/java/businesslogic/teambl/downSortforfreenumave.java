...
public class downSortforfreenumave {
     ...
     */
      ...
      */
      public static TeamVO median( ArrayList<TeamVO> arr, int left, int right ) {
		...
		if ( arr.get(left).getFreeNumave() <arr.get(center).getFreeNumave()  )
			swapRef( arr, left, center );
		if ( arr.get(left).getFreeNumave() <arr.get(right).getFreeNumave() )
			swapRef( arr, left, right );
		if ( arr.get(center).getFreeNumave() <arr.get(right).getFreeNumave() )
      ...
      */
      private static  void quickSort( ArrayList<TeamVO> arr, int left, int right ) {
		if ( left + CUTOFF <= right  ) {
			...
			for ( ; ; ) {
				while ( arr.get(++i).getFreeNumave()>pivot.getFreeNumave() ) ;
				while (arr.get(--j).getFreeNumave()<pivot.getFreeNumave() ) ;
				if ( i < j )
					swapRef( arr, i, j );
				else
					break;
      ...
      public static  void insertionSort( ArrayList<TeamVO> arr, int start, int end ) {
		...
		for ( int j = start + 1; j <= end; j++ ) {
			...
			for ( i = j; i > start && tmp.getFreeNumave()> arr.get(i - 1).getFreeNumave() ; i-- ) {
