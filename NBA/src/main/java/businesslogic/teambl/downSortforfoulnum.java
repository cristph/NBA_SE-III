...
public class downSortforfoulnum {
     ...
     */
      ...
      */
      public static TeamVO median( ArrayList<TeamVO> arr, int left, int right ) {
		...
		if ( arr.get(left).getFoulNum() <arr.get(center).getFoulNum()  )
			swapRef( arr, left, center );
		if ( arr.get(left).getFoulNum() <arr.get(right).getFoulNum() )
			swapRef( arr, left, right );
		if ( arr.get(center).getFoulNum()<arr.get(right).getFoulNum())
      ...
      */
      private static  void quickSort( ArrayList<TeamVO> arr, int left, int right ) {
		if ( left + CUTOFF <= right  ) {
			...
			for ( ; ; ) {
				while ( arr.get(++i).getFoulNum()>pivot.getFoulNum()) ;
				while (arr.get(--j).getFoulNum()<pivot.getFoulNum()) ;
				if ( i < j )
					swapRef( arr, i, j );
				else
					break;
      ...
      public static  void insertionSort( ArrayList<TeamVO> arr, int start, int end ) {
		...
		for ( int j = start + 1; j <= end; j++ ) {
			...
			for ( i = j; i > start && tmp.getFoulNum()> arr.get(i - 1).getFoulNum() ; i-- ) {
