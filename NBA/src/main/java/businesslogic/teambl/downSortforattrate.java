...
public class downSortforattrate {
     ...
     */
      ...
      */
      public static TeamVO median( ArrayList<TeamVO> arr, int left, int right ) {
		...
		if ( arr.get(left).getAttackrate() <arr.get(center).getAttackrate()  )
			swapRef( arr, left, center );
		if ( arr.get(left).getAttackrate() <arr.get(right).getAttackrate() )
			swapRef( arr, left, right );
		if ( arr.get(center).getAttackrate() <arr.get(right).getAttackrate())
      ...
      */
      private static  void quickSort( ArrayList<TeamVO> arr, int left, int right ) {
		if ( left + CUTOFF <= right  ) {
			...
			for ( ; ; ) {
				while ( arr.get(++i).getAttackrate()>pivot.getAttackrate() ) ;
				while (arr.get(--j).getAttackrate()<pivot.getAttackrate() ) ;
				if ( i < j )
					swapRef( arr, i, j );
				else
					break;
      ...
      public static  void insertionSort( ArrayList<TeamVO> arr, int start, int end ) {
		...
		for ( int j = start + 1; j <= end; j++ ) {
			...
			for ( i = j; i > start && tmp.getAttackrate()> arr.get(i - 1).getAttackrate() ; i-- ) {
