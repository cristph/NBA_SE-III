...
public class downSortforwinrate {
     ...
     */
      ...
      */
      public static TeamVO median( ArrayList<TeamVO> arr, int left, int right ) {
		...
		if ( arr.get(left).getWinrate() <arr.get(center).getWinrate()  )
			swapRef( arr, left, center );
		if ( arr.get(left).getWinrate()<arr.get(right).getWinrate() )
			swapRef( arr, left, right );
		if ( arr.get(center).getWinrate() <arr.get(right).getWinrate() )
      ...
      */
      private static  void quickSort( ArrayList<TeamVO> arr, int left, int right ) {
		if ( left + CUTOFF <= right  ) {
			...
			for ( ; ; ) {
				while ( arr.get(++i).getWinrate()>pivot.getWinrate()) ;
				while (arr.get(--j).getWinrate()<pivot.getWinrate() ) ;
				if ( i < j )
					swapRef( arr, i, j );
				else
					break;
      ...
      public static  void insertionSort( ArrayList<TeamVO> arr, int start, int end ) {
		...
		for ( int j = start + 1; j <= end; j++ ) {
			...
			for ( i = j; i > start && tmp.getWinrate()> arr.get(i - 1).getWinrate() ; i-- ) {
