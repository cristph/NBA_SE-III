...
public class downSortforerrornumave {
     ...
     */
      ...
      */
      public static TeamVO median( ArrayList<TeamVO> arr, int left, int right ) {
		...
		if ( arr.get(left).getErrorNumave() <arr.get(center).getErrorNumave()  )
			swapRef( arr, left, center );
		if ( arr.get(left).getErrorNumave() <arr.get(right).getErrorNumave() )
			swapRef( arr, left, right );
		if ( arr.get(center).getErrorNumave() <arr.get(right).getErrorNumave() )
      ...
      */
      private static  void quickSort( ArrayList<TeamVO> arr, int left, int right ) {
		if ( left + CUTOFF <= right  ) {
			...
			for ( ; ; ) {
				while ( arr.get(++i).getErrorNumave()>pivot.getErrorNumave() ) ;
				while (arr.get(--j).getErrorNumave()<pivot.getErrorNumave() ) ;
				if ( i < j )
					swapRef( arr, i, j );
				else
					break;
      ...
      public static  void insertionSort( ArrayList<TeamVO> arr, int start, int end ) {
		...
		for ( int j = start + 1; j <= end; j++ ) {
			...
			for ( i = j; i > start && tmp.getErrorNumave()> arr.get(i - 1).getErrorNumave(); i-- ) {
