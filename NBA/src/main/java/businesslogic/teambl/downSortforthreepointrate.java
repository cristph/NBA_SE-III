...
public class downSortforthreepointrate {
     ...
     */
      ...
      */
      public static TeamVO median( ArrayList<TeamVO> arr, int left, int right ) {
		...
		if ( arr.get(left).getThreepointrate() <arr.get(center).getThreepointrate()  )
			swapRef( arr, left, center );
		if ( arr.get(left).getThreepointrate() <arr.get(right).getThreepointrate())
			swapRef( arr, left, right );
		if ( arr.get(center).getThreepointrate() <arr.get(right).getThreepointrate() )
      ...
      */
      private static  void quickSort( ArrayList<TeamVO> arr, int left, int right ) {
		if ( left + CUTOFF <= right  ) {
			...
			for ( ; ; ) {
				while ( arr.get(++i).getThreepointrate()>pivot.getThreepointrate() ) ;
				while (arr.get(--j).getThreepointrate()<pivot.getThreepointrate()) ;
				if ( i < j )
					swapRef( arr, i, j );
				else
					break;
      ...
      public static  void insertionSort( ArrayList<TeamVO> arr, int start, int end ) {
		...
		for ( int j = start + 1; j <= end; j++ ) {
			...
			for ( i = j; i > start && tmp.getThreepointrate()> arr.get(i - 1).getThreepointrate() ; i-- ) {
