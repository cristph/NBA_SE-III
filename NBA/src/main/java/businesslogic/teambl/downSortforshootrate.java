...
public class downSortforshootrate {
     ...
     */
      ...
      */
      public static TeamVO median( ArrayList<TeamVO> arr, int left, int right ) {
		...
		if ( arr.get(left).getShootrate() <arr.get(center).getShootrate()  )
			swapRef( arr, left, center );
		if ( arr.get(left).getShootrate() <arr.get(right).getShootrate() )
			swapRef( arr, left, right );
		if ( arr.get(center).getShootrate() <arr.get(right).getShootrate() )
      ...
      */
      private static  void quickSort( ArrayList<TeamVO> arr, int left, int right ) {
		if ( left + CUTOFF <= right  ) {
			...
			for ( ; ; ) {
				while ( arr.get(++i).getShootrate()>pivot.getShootrate()) ;
				while (arr.get(--j).getShootrate()<pivot.getShootrate() ) ;
				if ( i < j )
					swapRef( arr, i, j );
				else
					break;
      ...
      public static  void insertionSort( ArrayList<TeamVO> arr, int start, int end ) {
		...
		for ( int j = start + 1; j <= end; j++ ) {
			...
			for ( i = j; i > start && tmp.getShootrate()> arr.get(i - 1).getShootrate() ; i-- ) {
