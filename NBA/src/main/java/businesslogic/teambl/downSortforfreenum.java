...
public class downSortforfreenum {
     ...
     */
      ...
      */
      public static TeamVO median( ArrayList<TeamVO> arr, int left, int right ) {
		...
		if ( arr.get(left).getFreeNum()<arr.get(center).getFreeNum()  )
			swapRef( arr, left, center );
		if ( arr.get(left).getFreeNum() <arr.get(right).getFreeNum())
			swapRef( arr, left, right );
		if ( arr.get(center).getFreeNum() <arr.get(right).getFreeNum() )
      ...
      */
      private static  void quickSort( ArrayList<TeamVO> arr, int left, int right ) {
		if ( left + CUTOFF <= right  ) {
			...
			for ( ; ; ) {
				while ( arr.get(++i).getFreeNum()>pivot.getFreeNum() ) ;
				while (arr.get(--j).getFreeNum()<pivot.getFreeNum() ) ;
				if ( i < j )
					swapRef( arr, i, j );
				else
					break;
      ...
      public static  void insertionSort( ArrayList<TeamVO> arr, int start, int end ) {
		...
		for ( int j = start + 1; j <= end; j++ ) {
			...
			for ( i = j; i > start && tmp.getFreeNum()> arr.get(i - 1).getFreeNum() ; i-- ) {
