...
public class downSortforthreepointnum {
     ...
     */
      ...
      */
      public static TeamVO median( ArrayList<TeamVO> arr, int left, int right ) {
		...
		if ( arr.get(left).getThreePointNum() <arr.get(center).getThreePointNum()  )
			swapRef( arr, left, center );
		if ( arr.get(left).getThreePointNum() <arr.get(right).getThreePointNum())
			swapRef( arr, left, right );
		if ( arr.get(center).getThreePointNum() <arr.get(right).getThreePointNum() )
      ...
      */
      private static  void quickSort( ArrayList<TeamVO> arr, int left, int right ) {
		if ( left + CUTOFF <= right  ) {
			...
			for ( ; ; ) {
				while ( arr.get(++i).getThreePointNum()>pivot.getThreePointNum() ) ;
				while (arr.get(--j).getThreePointNum()<pivot.getThreePointNum() ) ;
				if ( i < j )
					swapRef( arr, i, j );
				else
					break;
      ...
      public static  void insertionSort( ArrayList<TeamVO> arr, int start, int end ) {
		...
		for ( int j = start + 1; j <= end; j++ ) {
			...
			for ( i = j; i > start && tmp.getThreePointNum()> arr.get(i - 1).getThreePointNum() ; i-- ) {
