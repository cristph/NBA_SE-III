...
public class downSortforblocknumave {
     ...
     */
      ...
      */
      public static TeamVO median( ArrayList<TeamVO> arr, int left, int right ) {
		...
		if ( arr.get(left).getBlockNumave() <arr.get(center).getBlockNumave() )
			swapRef( arr, left, center );
		if ( arr.get(left).getBlockNumave() <arr.get(right).getBlockNumave() )
			swapRef( arr, left, right );
		if ( arr.get(center).getBlockNumave() <arr.get(right).getBlockNumave() )
      ...
      */
      private static  void quickSort( ArrayList<TeamVO> arr, int left, int right ) {
		if ( left + CUTOFF <= right  ) {
			...
			for ( ; ; ) {
				while ( arr.get(++i).getBlockNumave()>pivot.getBlockNumave()) ;
				while (arr.get(--j).getBlockNumave()<pivot.getBlockNumave() ) ;
				if ( i < j )
					swapRef( arr, i, j );
				else
					break;
      ...
      public static  void insertionSort( ArrayList<TeamVO> arr, int start, int end ) {
		...
		for ( int j = start + 1; j <= end; j++ ) {
			...
			for ( i = j; i > start && tmp.getBlockNumave()> arr.get(i - 1).getBlockNumave() ; i-- ) {
