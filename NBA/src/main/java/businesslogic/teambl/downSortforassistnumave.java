...
public class downSortforassistnumave {
     ...
     */
      ...
      */
      public static TeamVO median( ArrayList<TeamVO> arr, int left, int right ) {
		...
		if ( arr.get(left).getAssistNumave() <arr.get(center).getAssistNumave()  )
			swapRef( arr, left, center );
		if ( arr.get(left).getAssistNumave() <arr.get(right).getAssistNumave())
			swapRef( arr, left, right );
		if ( arr.get(center).getAssistNumave() <arr.get(right).getAssistNumave() )
      ...
      */
      private static  void quickSort( ArrayList<TeamVO> arr, int left, int right ) {
		if ( left + CUTOFF <= right  ) {
			...
			for ( ; ; ) {
				while ( arr.get(++i).getAssistNumave()>pivot.getAssistNumave() ) ;
				while (arr.get(--j).getAssistNumave()<pivot.getAssistNumave() ) ;
				if ( i < j )
					swapRef( arr, i, j );
				else
					break;
      ...
      public static  void insertionSort( ArrayList<TeamVO> arr, int start, int end ) {
		...
		for ( int j = start + 1; j <= end; j++ ) {
			...
			for ( i = j; i > start && tmp.getAssistNumave()> arr.get(i - 1).getAssistNumave() ; i-- ) {
