...
public class downSortforrebattnum {
     ...
     */
      ...
      */
      public static TeamVO median( ArrayList<TeamVO> arr, int left, int right ) {
		...
		if ( arr.get(left).getRebAttNum() <arr.get(center).getRebAttNum() )
			swapRef( arr, left, center );
		if ( arr.get(left).getRebAttNum() <arr.get(right).getRebAttNum() )
			swapRef( arr, left, right );
		if ( arr.get(center).getRebAttNum()<arr.get(right).getRebAttNum() )
      ...
      */
      private static  void quickSort( ArrayList<TeamVO> arr, int left, int right ) {
		if ( left + CUTOFF <= right  ) {
			...
			for ( ; ; ) {
				while ( arr.get(++i).getRebAttNum()>pivot.getRebAttNum() ) ;
				while (arr.get(--j).getRebAttNum()<pivot.getRebAttNum() ) ;
				if ( i < j )
					swapRef( arr, i, j );
				else
					break;
      ...
      public static  void insertionSort( ArrayList<TeamVO> arr, int start, int end ) {
		...
		for ( int j = start + 1; j <= end; j++ ) {
			...
			for ( i = j; i > start && tmp.getRebAttNum()> arr.get(i - 1).getRebAttNum() ; i-- ) {
