...
public class downSortforfreehitnumave {
     ...
     */
      ...
      */
      public static TeamVO median( ArrayList<TeamVO> arr, int left, int right ) {
		...
		if ( arr.get(left).getFreeHitNumave() <arr.get(center).getFreeHitNumave()  )
			swapRef( arr, left, center );
		if ( arr.get(left).getFreeHitNumave() <arr.get(right).getFreeHitNumave() )
			swapRef( arr, left, right );
		if ( arr.get(center).getFreeHitNumave() <arr.get(right).getFreeHitNumave() )
      ...
      */
      private static  void quickSort( ArrayList<TeamVO> arr, int left, int right ) {
		if ( left + CUTOFF <= right  ) {
			...
			for ( ; ; ) {
				while ( arr.get(++i).getFreeHitNumave()>pivot.getFreeHitNumave()) ;
				while (arr.get(--j).getFreeHitNumave()<pivot.getFreeHitNumave() ) ;
				if ( i < j )
					swapRef( arr, i, j );
				else
					break;
      ...
      public static  void insertionSort( ArrayList<TeamVO> arr, int start, int end ) {
		...
		for ( int j = start + 1; j <= end; j++ ) {
			...
			for ( i = j; i > start && tmp.getFreeHitNumave()> arr.get(i - 1).getFreeHitNumave() ; i-- ) {
