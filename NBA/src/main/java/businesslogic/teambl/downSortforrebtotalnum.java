...
public class downSortforrebtotalnum {
     ...
     */
      ...
      */
      public static TeamVO median( ArrayList<TeamVO> arr, int left, int right ) {
		...
		if ( arr.get(left).getRebTotalNum() <arr.get(center).getRebTotalNum()  )
			swapRef( arr, left, center );
		if ( arr.get(left).getRebTotalNum()<arr.get(right).getRebTotalNum() )
			swapRef( arr, left, right );
		if ( arr.get(center).getRebTotalNum() <arr.get(right).getRebTotalNum() )
      ...
      */
      private static  void quickSort( ArrayList<TeamVO> arr, int left, int right ) {
		if ( left + CUTOFF <= right  ) {
			...
			for ( ; ; ) {
				while ( arr.get(++i).getRebTotalNum()>pivot.getRebTotalNum()) ;
				while (arr.get(--j).getRebTotalNum()<pivot.getRebTotalNum() ) ;
				if ( i < j )
					swapRef( arr, i, j );
				else
					break;
      ...
      public static  void insertionSort( ArrayList<TeamVO> arr, int start, int end ) {
		...
		for ( int j = start + 1; j <= end; j++ ) {
			...
			for ( i = j; i > start && tmp.getRebTotalNum()> arr.get(i - 1).getRebTotalNum() ; i-- ) {
