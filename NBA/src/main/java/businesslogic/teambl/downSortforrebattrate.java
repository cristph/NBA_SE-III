...
public class downSortforrebattrate {
     ...
     */
      ...
      */
      public static TeamVO median( ArrayList<TeamVO> arr, int left, int right ) {
		...
		if ( arr.get(left).getRebattrate() <arr.get(center).getRebattrate()  )
			swapRef( arr, left, center );
		if ( arr.get(left).getRebattrate() <arr.get(right).getRebattrate() )
			swapRef( arr, left, right );
		if ( arr.get(center).getRebattrate() <arr.get(right).getRebattrate() )
      ...
      */
      private static  void quickSort( ArrayList<TeamVO> arr, int left, int right ) {
		if ( left + CUTOFF <= right  ) {
			...
			for ( ; ; ) {
				while ( arr.get(++i).getRebattrate()>pivot.getRebattrate() ) ;
				while (arr.get(--j).getRebattrate()<pivot.getRebattrate()) ;
				if ( i < j )
					swapRef( arr, i, j );
				else
					break;
      ...
      public static  void insertionSort( ArrayList<TeamVO> arr, int start, int end ) {
		...
		for ( int j = start + 1; j <= end; j++ ) {
			...
			for ( i = j; i > start && tmp.getRebattrate()> arr.get(i - 1).getRebattrate() ; i-- ) {
