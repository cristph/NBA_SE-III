...
public class downSortforhitshootnumave {
     ...
     */
      ...
      */
      public static TeamVO median( ArrayList<TeamVO> arr, int left, int right ) {
		...
		if ( arr.get(left).getHitShootNumave() <arr.get(center).getHitShootNumave()  )
			swapRef( arr, left, center );
		if ( arr.get(left).getHitShootNumave() <arr.get(right).getHitShootNumave() )
			swapRef( arr, left, right );
		if ( arr.get(center).getHitShootNumave() <arr.get(right).getHitShootNumave() )
      ...
      */
      private static  void quickSort( ArrayList<TeamVO> arr, int left, int right ) {
		if ( left + CUTOFF <= right  ) {
			...
			for ( ; ; ) {
				while ( arr.get(++i).getHitShootNumave()>pivot.getHitShootNumave() ) ;
				while (arr.get(--j).getHitShootNumave()<pivot.getHitShootNumave() ) ;
				if ( i < j )
					swapRef( arr, i, j );
				else
					break;
      ...
      public static  void insertionSort( ArrayList<TeamVO> arr, int start, int end ) {
		...
		for ( int j = start + 1; j <= end; j++ ) {
			...
			for ( i = j; i > start && tmp.getHitShootNumave()> arr.get(i - 1).getHitShootNumave() ; i-- ) {
