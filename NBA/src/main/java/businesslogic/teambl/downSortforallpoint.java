...
public class downSortforallpoint {
	     ...
	     */
	      ...
	      */
	      public static TeamVO median( ArrayList<TeamVO> arr, int left, int right ) {
			...
			if ( arr.get(left).getAllpoint() <arr.get(center).getAllpoint()  )
				swapRef( arr, left, center );
			if ( arr.get(left).getAllpoint() <arr.get(right).getAllpoint() )
				swapRef( arr, left, right );
			if ( arr.get(center).getAllpoint() <arr.get(right).getAllpoint() )
	      ...
	      */
	      private static  void quickSort( ArrayList<TeamVO> arr, int left, int right ) {
			if ( left + CUTOFF <= right  ) {
				...
				for ( ; ; ) {
					while ( arr.get(++i).getAllpoint()>pivot.getAllpoint() ) ;
					while (arr.get(--j).getAllpoint()<pivot.getAllpoint() ) ;
					if ( i < j )
						swapRef( arr, i, j );
					else
						break;
	      ...
	      public static  void insertionSort( ArrayList<TeamVO> arr, int start, int end ) {
			...
			for ( int j = start + 1; j <= end; j++ ) {
				...
				for ( i = j; i > start && tmp.getAllpoint()> arr.get(i - 1).getAllpoint() ; i-- ) {
