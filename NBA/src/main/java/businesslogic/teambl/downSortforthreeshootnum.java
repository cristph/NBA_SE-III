...
public class downSortforthreeshootnum {
     ...
     */
      ...
      */
      public static TeamVO median( ArrayList<TeamVO> arr, int left, int right ) {
		...
		if ( arr.get(left).getThreeShootNum() <arr.get(center).getThreeShootNum()  )
			swapRef( arr, left, center );
		if ( arr.get(left).getThreeShootNum() <arr.get(right).getThreeShootNum() )
			swapRef( arr, left, right );
		if ( arr.get(center).getThreeShootNum() <arr.get(right).getThreeShootNum() )
      ...
      */
      private static  void quickSort( ArrayList<TeamVO> arr, int left, int right ) {
		if ( left + CUTOFF <= right  ) {
			...
			for ( ; ; ) {
				while ( arr.get(++i).getThreeShootNum()>pivot.getThreeShootNum() ) ;
				while (arr.get(--j).getThreeShootNum()<pivot.getThreeShootNum()) ;
				if ( i < j )
					swapRef( arr, i, j );
				else
					break;
      ...
      public static  void insertionSort( ArrayList<TeamVO> arr, int start, int end ) {
		...
		for ( int j = start + 1; j <= end; j++ ) {
			...
			for ( i = j; i > start && tmp.getThreeShootNum()> arr.get(i - 1).getThreeShootNum() ; i-- ) {
