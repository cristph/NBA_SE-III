...
public class ErrorRateUPSort {
	...
	public PlayerHighInfo median( ArrayList<PlayerHighInfo> arr, int left, int right ) {
		...
		if ( arr.get(left).getFaultEfficient()>arr.get(center).getFaultEfficient() )
			swapRef( arr, left, center );
		if ( arr.get(left).getFaultEfficient()>arr.get(right).getFaultEfficient() )
			swapRef( arr, left, right );
		if ( arr.get(center).getFaultEfficient()>arr.get(right).getFaultEfficient() )
	...
	private void quickSort( ArrayList<PlayerHighInfo> arr, int left, int right ) {
		...
		if ( left + CUTOFF <= right  ) {
			...
			for ( ; ; ) {
				while ( arr.get(++i).getFaultEfficient()< pivot.getFaultEfficient()  ) ;
				while ( arr.get(--j).getFaultEfficient()> pivot.getFaultEfficient() ) ;
				if ( i < j )
					swapRef( arr, i, j );
				else
					break;
	...
	public void insertionSort( ArrayList<PlayerHighInfo> arr, int start, int end ) {
		...
		for ( int j = start + 1; j <= end; j++ ) {
			...
			for ( i = j; i > start && tmp.getFaultEfficient()<arr.get(i - 1).getFaultEfficient(); i-- ) {
