...
public class ErrorRateUPSort {
	...
	public PlayerInfoVO median( ArrayList<PlayerInfoVO> arr, int left, int right ) {
		...
		if ( arr.get(left).getErrorRate()>arr.get(center).getErrorRate() )
			swapRef( arr, left, center );
		if ( arr.get(left).getErrorRate()>arr.get(right).getErrorRate() )
			swapRef( arr, left, right );
		if ( arr.get(center).getErrorRate()>arr.get(right).getErrorRate() )
	...
	private void quickSort( ArrayList<PlayerInfoVO> arr, int left, int right ) {
		...
		if ( left + CUTOFF <= right  ) {
			...
			for ( ; ; ) {
				while ( arr.get(++i).getErrorRate()< pivot.getErrorRate()  ) ;
				while ( arr.get(--j).getErrorRate()> pivot.getErrorRate() ) ;
				if ( i < j )
					swapRef( arr, i, j );
				else
					break;
	...
	public void insertionSort( ArrayList<PlayerInfoVO> arr, int start, int end ) {
		...
		for ( int j = start + 1; j <= end; j++ ) {
			...
			for ( i = j; i > start && tmp.getErrorRate()<arr.get(i - 1).getErrorRate(); i-- ) {
