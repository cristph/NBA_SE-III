...
public class TimeUPSort {
	...
	public PlayerNormalInfo median( ArrayList<PlayerNormalInfo> arr, int left, int right ) {
		...
		if ( arr.get(left).getMinute()>arr.get(center).getMinute() )
			swapRef( arr, left, center );
		if ( arr.get(left).getMinute()>arr.get(right).getMinute() )
			swapRef( arr, left, right );
		if ( arr.get(center).getMinute()>arr.get(right).getMinute() )
	...
	private void quickSort( ArrayList<PlayerNormalInfo> arr, int left, int right ) {
		...
		if ( left + CUTOFF <= right  ) {
			...
			for ( ; ; ) {
				while ( arr.get(++i).getMinute()< pivot.getMinute()  ) ;
				while ( arr.get(--j).getMinute()> pivot.getMinute() ) ;
				if ( i < j )
					swapRef( arr, i, j );
				else
					break;
	...
	public void insertionSort( ArrayList<PlayerNormalInfo> arr, int start, int end ) {
		...
		for ( int j = start + 1; j <= end; j++ ) {
			...
			for ( i = j; i > start && tmp.getMinute()<arr.get(i - 1).getMinute(); i-- ) {
