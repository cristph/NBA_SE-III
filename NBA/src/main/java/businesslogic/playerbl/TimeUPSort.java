...
public class TimeUPSort {
	...
	public PlayerInfoVO median( ArrayList<PlayerInfoVO> arr, int left, int right ) {
		...
		if ( arr.get(left).getTime()>arr.get(center).getTime() )
			swapRef( arr, left, center );
		if ( arr.get(left).getTime()>arr.get(right).getTime() )
			swapRef( arr, left, right );
		if ( arr.get(center).getTime()>arr.get(right).getTime() )
	...
	private void quickSort( ArrayList<PlayerInfoVO> arr, int left, int right ) {
		...
		if ( left + CUTOFF <= right  ) {
			...
			for ( ; ; ) {
				while ( arr.get(++i).getTime()< pivot.getTime()  ) ;
				while ( arr.get(--j).getTime()> pivot.getTime() ) ;
				if ( i < j )
					swapRef( arr, i, j );
				else
					break;
	...
	public void insertionSort( ArrayList<PlayerInfoVO> arr, int start, int end ) {
		...
		for ( int j = start + 1; j <= end; j++ ) {
			...
			for ( i = j; i > start && tmp.getTime()<arr.get(i - 1).getTime(); i-- ) {
