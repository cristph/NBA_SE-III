...
public class GMSCUPSort {
	...
	public PlayerInfoVO median( ArrayList<PlayerInfoVO> arr, int left, int right ) {
		...
		if ( arr.get(left).getGMSC()>arr.get(center).getGMSC() )
			swapRef( arr, left, center );
		if ( arr.get(left).getGMSC()>arr.get(right).getGMSC() )
			swapRef( arr, left, right );
		if ( arr.get(center).getGMSC()>arr.get(right).getGMSC() )
	...
	private void quickSort( ArrayList<PlayerInfoVO> arr, int left, int right ) {
		...
		if ( left + CUTOFF <= right  ) {
			...
			for ( ; ; ) {
				while ( arr.get(++i).getGMSC()< pivot.getGMSC()  ) ;
				while ( arr.get(--j).getGMSC()> pivot.getGMSC() ) ;
				if ( i < j )
					swapRef( arr, i, j );
				else
					break;
	...
	public void insertionSort( ArrayList<PlayerInfoVO> arr, int start, int end ) {
		...
		for ( int j = start + 1; j <= end; j++ ) {
			...
			for ( i = j; i > start && tmp.getGMSC()<arr.get(i - 1).getGMSC(); i-- ) {
