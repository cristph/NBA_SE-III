...
public class RealHitRateDSort {
	...
	public PlayerHighInfo median( ArrayList<PlayerHighInfo> arr, int left, int right ) {
		...
		if ( arr.get(left).getRealShot()<arr.get(center).getRealShot() )
			swapRef( arr, left, center );
		if ( arr.get(left).getRealShot()<arr.get(right).getRealShot() )
			swapRef( arr, left, right );
		if ( arr.get(center).getRealShot()<arr.get(right).getRealShot() )
	...
	private void quickSort( ArrayList<PlayerHighInfo> arr, int left, int right ) {
		...
		if ( left + CUTOFF <= right  ) {
			...
			for ( ; ; ) {
				while ( arr.get(++i).getRealShot()> pivot.getRealShot()  ) ;
				while ( arr.get(--j).getRealShot()< pivot.getRealShot() ) ;
				if ( i < j )
					swapRef( arr, i, j );
				else
					break;
	...
	public void insertionSort( ArrayList<PlayerHighInfo> arr, int start, int end ) {
		...
		for ( int j = start + 1; j <= end; j++ ) {
			...
			for ( i = j; i > start && tmp.getRealShot()>arr.get(i - 1).getRealShot(); i-- ) {
