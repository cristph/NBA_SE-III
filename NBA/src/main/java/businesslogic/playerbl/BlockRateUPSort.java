...
public class BlockRateUPSort {
	...
	public PlayerInfoVO median( ArrayList<PlayerInfoVO> arr, int left, int right ) {
		...
		if ( arr.get(left).getBlockRate()>arr.get(center).getBlockRate() )
			swapRef( arr, left, center );
		if ( arr.get(left).getBlockRate()>arr.get(right).getBlockRate() )
			swapRef( arr, left, right );
		if ( arr.get(center).getBlockRate()>arr.get(right).getBlockRate() )
	...
	private void quickSort( ArrayList<PlayerInfoVO> arr, int left, int right ) {
		...
		if ( left + CUTOFF <= right  ) {
			...
			for ( ; ; ) {
				while ( arr.get(++i).getBlockRate()< pivot.getBlockRate()  ) ;
				while ( arr.get(--j).getBlockRate()> pivot.getBlockRate() ) ;
				if ( i < j )
					swapRef( arr, i, j );
				else
					break;
	...
	public void insertionSort( ArrayList<PlayerInfoVO> arr, int start, int end ) {
		...
		for ( int j = start + 1; j <= end; j++ ) {
			...
			for ( i = j; i > start && tmp.getBlockRate()<arr.get(i - 1).getBlockRate(); i-- ) {
