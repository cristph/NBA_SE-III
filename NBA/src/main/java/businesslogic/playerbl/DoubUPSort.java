...
public class DoubUPSort {
	...
	public PlayerInfoVO median( ArrayList<PlayerInfoVO> arr, int left, int right ) {
		...
		if ( arr.get(left).getDoub()>arr.get(center).getDoub() )
			swapRef( arr, left, center );
		if ( arr.get(left).getDoub()>arr.get(right).getDoub() )
			swapRef( arr, left, right );
		if ( arr.get(center).getDoub()>arr.get(right).getDoub() )
	...
	private void quickSort( ArrayList<PlayerInfoVO> arr, int left, int right ) {
		...
		if ( left + CUTOFF <= right  ) {
			...
			for ( ; ; ) {
				while ( arr.get(++i).getDoub()< pivot.getDoub()  ) ;
				while ( arr.get(--j).getDoub()> pivot.getDoub() ) ;
				if ( i < j )
					swapRef( arr, i, j );
				else
					break;
	...
	public void insertionSort( ArrayList<PlayerInfoVO> arr, int start, int end ) {
		...
		for ( int j = start + 1; j <= end; j++ ) {
			...
			for ( i = j; i > start && tmp.getDoub()<arr.get(i - 1).getDoub(); i-- ) {
