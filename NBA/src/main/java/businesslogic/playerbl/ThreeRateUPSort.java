...
public class ThreeRateUPSort {
	...
	public PlayerInfoVO median( ArrayList<PlayerInfoVO> arr, int left, int right ) {
		...
		if ( arr.get(left).getThreeRate()>arr.get(center).getThreeRate() )
			swapRef( arr, left, center );
		if ( arr.get(left).getThreeRate()>arr.get(right).getThreeRate() )
			swapRef( arr, left, right );
		if ( arr.get(center).getThreeRate()>arr.get(right).getThreeRate() )
	...
	private void quickSort( ArrayList<PlayerInfoVO> arr, int left, int right ) {
		...
		if ( left + CUTOFF <= right  ) {
			...
			for ( ; ; ) {
				while ( arr.get(++i).getThreeRate()< pivot.getThreeRate()  ) ;
				while ( arr.get(--j).getThreeRate()> pivot.getThreeRate() ) ;
				if ( i < j )
					swapRef( arr, i, j );
				else
					break;
	...
	public void insertionSort( ArrayList<PlayerInfoVO> arr, int start, int end ) {
		...
		for ( int j = start + 1; j <= end; j++ ) {
			...
			for ( i = j; i > start && tmp.getThreeRate()<arr.get(i - 1).getThreeRate(); i-- ) {
