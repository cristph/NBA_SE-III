...
public class ThreeRateUPSort {
	...
	public PlayerNormalInfo median( ArrayList<PlayerNormalInfo> arr, int left, int right ) {
		...
		if ( arr.get(left).getThree()>arr.get(center).getThree() )
			swapRef( arr, left, center );
		if ( arr.get(left).getThree()>arr.get(right).getThree() )
			swapRef( arr, left, right );
		if ( arr.get(center).getThree()>arr.get(right).getThree() )
	...
	private void quickSort( ArrayList<PlayerNormalInfo> arr, int left, int right ) {
		...
		if ( left + CUTOFF <= right  ) {
			...
			for ( ; ; ) {
				while ( arr.get(++i).getThree()< pivot.getThree()  ) ;
				while ( arr.get(--j).getThree()> pivot.getThree() ) ;
				if ( i < j )
					swapRef( arr, i, j );
				else
					break;
	...
	public void insertionSort( ArrayList<PlayerNormalInfo> arr, int start, int end ) {
		...
		for ( int j = start + 1; j <= end; j++ ) {
			...
			for ( i = j; i > start && tmp.getThree()<arr.get(i - 1).getThree(); i-- ) {
