...
public class BlockNumUPSort {
	...
	public PlayerNormalInfo median( ArrayList<PlayerNormalInfo> arr, int left, int right ) {
		...
		if ( arr.get(left).getBlockShot()>arr.get(center).getBlockShot() )
			swapRef( arr, left, center );
		if ( arr.get(left).getBlockShot()>arr.get(right).getBlockShot() )
			swapRef( arr, left, right );
		if ( arr.get(center).getBlockShot()>arr.get(right).getBlockShot() )
	...
	private void quickSort( ArrayList<PlayerNormalInfo> arr, int left, int right ) {
		...
		if ( left + CUTOFF <= right  ) {
			...
			for ( ; ; ) {
				while ( arr.get(++i).getBlockShot()< pivot.getBlockShot()  ) ;
				while ( arr.get(--j).getBlockShot()> pivot.getBlockShot() ) ;
				if ( i < j )
					swapRef( arr, i, j );
				else
					break;
	...
	public void insertionSort( ArrayList<PlayerNormalInfo> arr, int start, int end ) {
		...
		for ( int j = start + 1; j <= end; j++ ) {
			...
			for ( i = j; i > start && tmp.getBlockShot()<arr.get(i - 1).getBlockShot(); i-- ) {
