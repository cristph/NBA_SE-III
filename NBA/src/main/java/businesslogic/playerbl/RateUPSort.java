...
public class RateUPSort {
	...
	public PlayerInfoVO median( ArrayList<PlayerInfoVO> arr, int left, int right ) {
		...
		if ( arr.get(left).getRate()>arr.get(center).getRate() )
			swapRef( arr, left, center );
		if ( arr.get(left).getRate()>arr.get(right).getRate() )
			swapRef( arr, left, right );
		if ( arr.get(center).getRate()>arr.get(right).getRate() )
	...
	private void quickSort( ArrayList<PlayerInfoVO> arr, int left, int right ) {
		...
		if ( left + CUTOFF <= right  ) {
			...
			for ( ; ; ) {
				while ( arr.get(++i).getRate()< pivot.getRate()  ) ;
				while ( arr.get(--j).getRate()> pivot.getRate() ) ;
				if ( i < j )
					swapRef( arr, i, j );
				else
					break;
	...
	public void insertionSort( ArrayList<PlayerInfoVO> arr, int start, int end ) {
		...
		for ( int j = start + 1; j <= end; j++ ) {
			...
			for ( i = j; i > start && tmp.getRate()<arr.get(i - 1).getRate(); i-- ) {
