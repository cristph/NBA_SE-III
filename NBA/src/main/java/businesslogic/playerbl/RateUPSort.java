...
public class RateUPSort {
	...
	public PlayerNormalInfo median( ArrayList<PlayerNormalInfo> arr, int left, int right ) {
		...
		if ( arr.get(left).getEfficiency()>arr.get(center).getEfficiency() )
			swapRef( arr, left, center );
		if ( arr.get(left).getEfficiency()>arr.get(right).getEfficiency() )
			swapRef( arr, left, right );
		if ( arr.get(center).getEfficiency()>arr.get(right).getEfficiency() )
	...
	private void quickSort( ArrayList<PlayerNormalInfo> arr, int left, int right ) {
		...
		if ( left + CUTOFF <= right  ) {
			...
			for ( ; ; ) {
				while ( arr.get(++i).getEfficiency()< pivot.getEfficiency()  ) ;
				while ( arr.get(--j).getEfficiency()> pivot.getEfficiency() ) ;
				if ( i < j )
					swapRef( arr, i, j );
				else
					break;
	...
	public void insertionSort( ArrayList<PlayerNormalInfo> arr, int start, int end ) {
		...
		for ( int j = start + 1; j <= end; j++ ) {
			...
			for ( i = j; i > start && tmp.getEfficiency()<arr.get(i - 1).getEfficiency(); i-- ) {
