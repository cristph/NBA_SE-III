...
public class FirstTimeUPSort {
	...
	public PlayerInfoVO median( ArrayList<PlayerInfoVO> arr, int left, int right ) {
		...
		if ( arr.get(left).getFirstTime()>arr.get(center).getFirstTime() )
			swapRef( arr, left, center );
		if ( arr.get(left).getFirstTime()>arr.get(right).getFirstTime() )
			swapRef( arr, left, right );
		if ( arr.get(center).getFirstTime()>arr.get(right).getFirstTime() )
	...
	private void quickSort( ArrayList<PlayerInfoVO> arr, int left, int right ) {
		...
		if ( left + CUTOFF <= right  ) {
			...
			for ( ; ; ) {
				while ( arr.get(++i).getFirstTime()< pivot.getFirstTime()  ) ;
				while ( arr.get(--j).getFirstTime()> pivot.getFirstTime() ) ;
				if ( i < j )
					swapRef( arr, i, j );
				else
					break;
	...
	public void insertionSort( ArrayList<PlayerInfoVO> arr, int start, int end ) {
		...
		for ( int j = start + 1; j <= end; j++ ) {
			...
			for ( i = j; i > start && tmp.getFirstTime()<arr.get(i - 1).getFirstTime(); i-- ) {
