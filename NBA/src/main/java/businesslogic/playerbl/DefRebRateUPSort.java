...
public class DefRebRateUPSort {
	...
	public PlayerInfoVO median( ArrayList<PlayerInfoVO> arr, int left, int right ) {
		...
		if ( arr.get(left).getDefRebRate()>arr.get(center).getDefRebRate() )
			swapRef( arr, left, center );
		if ( arr.get(left).getDefRebRate()>arr.get(right).getDefRebRate() )
			swapRef( arr, left, right );
		if ( arr.get(center).getDefRebRate()>arr.get(right).getDefRebRate() )
	...
	private void quickSort( ArrayList<PlayerInfoVO> arr, int left, int right ) {
		...
		if ( left + CUTOFF <= right  ) {
			...
			for ( ; ; ) {
				while ( arr.get(++i).getDefRebRate()< pivot.getDefRebRate()  ) ;
				while ( arr.get(--j).getDefRebRate()> pivot.getDefRebRate() ) ;
				if ( i < j )
					swapRef( arr, i, j );
				else
					break;
	...
	public void insertionSort( ArrayList<PlayerInfoVO> arr, int start, int end ) {
		...
		for ( int j = start + 1; j <= end; j++ ) {
			...
			for ( i = j; i > start && tmp.getDefRebRate()<arr.get(i - 1).getDefRebRate(); i-- ) {
