...
public class DefRebRateUPSort {
	...
	public PlayerHighInfo median( ArrayList<PlayerHighInfo> arr, int left, int right ) {
		...
		if ( arr.get(left).getDefendReboundEfficient()>arr.get(center).getDefendReboundEfficient() )
			swapRef( arr, left, center );
		if ( arr.get(left).getDefendReboundEfficient()>arr.get(right).getDefendReboundEfficient() )
			swapRef( arr, left, right );
		if ( arr.get(center).getDefendReboundEfficient()>arr.get(right).getDefendReboundEfficient() )
	...
	private void quickSort( ArrayList<PlayerHighInfo> arr, int left, int right ) {
		...
		if ( left + CUTOFF <= right  ) {
			...
			for ( ; ; ) {
				while ( arr.get(++i).getDefendReboundEfficient()< pivot.getDefendReboundEfficient()  ) ;
				while ( arr.get(--j).getDefendReboundEfficient()> pivot.getDefendReboundEfficient() ) ;
				if ( i < j )
					swapRef( arr, i, j );
				else
					break;
	...
	public void insertionSort( ArrayList<PlayerHighInfo> arr, int start, int end ) {
		...
		for ( int j = start + 1; j <= end; j++ ) {
			...
			for ( i = j; i > start && tmp.getDefendReboundEfficient()<arr.get(i - 1).getDefendReboundEfficient(); i-- ) {
