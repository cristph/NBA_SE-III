...
public class FoulNumUPSort {
	...
	public PlayerNormalInfo median( ArrayList<PlayerNormalInfo> arr, int left, int right ) {
		...
		if ( arr.get(left).getFoul()>arr.get(center).getFoul() )
			swapRef( arr, left, center );
		if ( arr.get(left).getFoul()>arr.get(right).getFoul() )
			swapRef( arr, left, right );
		if ( arr.get(center).getFoul()>arr.get(right).getFoul() )
	...
	private void quickSort( ArrayList<PlayerNormalInfo> arr, int left, int right ) {
		...
		if ( left + CUTOFF <= right  ) {
			...
			for ( ; ; ) {
				while ( arr.get(++i).getFoul()< pivot.getFoul()  ) ;
				while ( arr.get(--j).getFoul()> pivot.getFoul() ) ;
				if ( i < j )
					swapRef( arr, i, j );
				else
					break;
	...
	public void insertionSort( ArrayList<PlayerNormalInfo> arr, int start, int end ) {
		...
		for ( int j = start + 1; j <= end; j++ ) {
			...
			for ( i = j; i > start && tmp.getFoul()<arr.get(i - 1).getFoul(); i-- ) {
