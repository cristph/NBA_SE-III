...
public class ShootingUPSort {
	...
	public PlayerInfoVO median( ArrayList<PlayerInfoVO> arr, int left, int right ) {
		...
		if ( arr.get(left).getShooting()>arr.get(center).getShooting() )
			swapRef( arr, left, center );
		if ( arr.get(left).getShooting()>arr.get(right).getShooting() )
			swapRef( arr, left, right );
		if ( arr.get(center).getShooting()>arr.get(right).getShooting() )
	...
	private void quickSort( ArrayList<PlayerInfoVO> arr, int left, int right ) {
		...
		if ( left + CUTOFF <= right  ) {
			...
			for ( ; ; ) {
				while ( arr.get(++i).getShooting()< pivot.getShooting()  ) ;
				while ( arr.get(--j).getShooting()> pivot.getShooting() ) ;
				if ( i < j )
					swapRef( arr, i, j );
				else
					break;
	...
	public void insertionSort( ArrayList<PlayerInfoVO> arr, int start, int end ) {
		...
		for ( int j = start + 1; j <= end; j++ ) {
			...
			for ( i = j; i > start && tmp.getShooting()<arr.get(i - 1).getShooting(); i-- ) {
