...
public class ShootingUPSort {
	...
	public PlayerNormalInfo median( ArrayList<PlayerNormalInfo> arr, int left, int right ) {
		...
		if ( arr.get(left).getShot()>arr.get(center).getShot() )
			swapRef( arr, left, center );
		if ( arr.get(left).getShot()>arr.get(right).getShot() )
			swapRef( arr, left, right );
		if ( arr.get(center).getShot()>arr.get(right).getShot() )
	...
	private void quickSort( ArrayList<PlayerNormalInfo> arr, int left, int right ) {
		...
		if ( left + CUTOFF <= right  ) {
			...
			for ( ; ; ) {
				while ( arr.get(++i).getShot()< pivot.getShot()  ) ;
				while ( arr.get(--j).getShot()> pivot.getShot() ) ;
				if ( i < j )
					swapRef( arr, i, j );
				else
					break;
	...
	public void insertionSort( ArrayList<PlayerNormalInfo> arr, int start, int end ) {
		...
		for ( int j = start + 1; j <= end; j++ ) {
			...
			for ( i = j; i > start && tmp.getShot()<arr.get(i - 1).getShot(); i-- ) {
