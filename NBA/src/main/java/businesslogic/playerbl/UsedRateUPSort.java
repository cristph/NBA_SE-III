...
public class UsedRateUPSort {
	...
	public PlayerInfoVO median( ArrayList<PlayerInfoVO> arr, int left, int right ) {
		...
		if ( arr.get(left).getUsedRate()>arr.get(center).getUsedRate() )
			swapRef( arr, left, center );
		if ( arr.get(left).getUsedRate()>arr.get(right).getUsedRate() )
			swapRef( arr, left, right );
		if ( arr.get(center).getUsedRate()>arr.get(right).getUsedRate() )
	...
	private void quickSort( ArrayList<PlayerInfoVO> arr, int left, int right ) {
		...
		if ( left + CUTOFF <= right  ) {
			...
			for ( ; ; ) {
				while ( arr.get(++i).getUsedRate()< pivot.getUsedRate()  ) ;
				while ( arr.get(--j).getUsedRate()> pivot.getUsedRate() ) ;
				if ( i < j )
					swapRef( arr, i, j );
				else
					break;
	...
	public void insertionSort( ArrayList<PlayerInfoVO> arr, int start, int end ) {
		...
		for ( int j = start + 1; j <= end; j++ ) {
			...
			for ( i = j; i > start && tmp.getUsedRate()<arr.get(i - 1).getUsedRate(); i-- ) {
