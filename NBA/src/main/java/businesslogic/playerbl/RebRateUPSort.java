...
public class RebRateUPSort {
	...
	public PlayerHighInfo median( ArrayList<PlayerHighInfo> arr, int left, int right ) {
		...
		if ( arr.get(left).getReboundEfficient()>arr.get(center).getReboundEfficient() )
			swapRef( arr, left, center );
		if ( arr.get(left).getReboundEfficient()>arr.get(right).getReboundEfficient() )
			swapRef( arr, left, right );
		if ( arr.get(center).getReboundEfficient()>arr.get(right).getReboundEfficient() )
	...
	private void quickSort( ArrayList<PlayerHighInfo> arr, int left, int right ) {
		...
		if ( left + CUTOFF <= right  ) {
			...
			for ( ; ; ) {
				while ( arr.get(++i).getReboundEfficient()< pivot.getReboundEfficient()  ) ;
				while ( arr.get(--j).getReboundEfficient()> pivot.getReboundEfficient() ) ;
				if ( i < j )
					swapRef( arr, i, j );
				else
					break;
	...
	public void insertionSort( ArrayList<PlayerHighInfo> arr, int start, int end ) {
		...
		for ( int j = start + 1; j <= end; j++ ) {
			...
			for ( i = j; i > start && tmp.getReboundEfficient()<arr.get(i - 1).getReboundEfficient(); i-- ) {
