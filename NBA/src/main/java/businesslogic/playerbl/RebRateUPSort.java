...
public class RebRateUPSort {
	...
	public PlayerInfoVO median( ArrayList<PlayerInfoVO> arr, int left, int right ) {
		...
		if ( arr.get(left).getRebRate()>arr.get(center).getRebRate() )
			swapRef( arr, left, center );
		if ( arr.get(left).getRebRate()>arr.get(right).getRebRate() )
			swapRef( arr, left, right );
		if ( arr.get(center).getRebRate()>arr.get(right).getRebRate() )
	...
	private void quickSort( ArrayList<PlayerInfoVO> arr, int left, int right ) {
		...
		if ( left + CUTOFF <= right  ) {
			...
			for ( ; ; ) {
				while ( arr.get(++i).getRebRate()< pivot.getRebRate()  ) ;
				while ( arr.get(--j).getRebRate()> pivot.getRebRate() ) ;
				if ( i < j )
					swapRef( arr, i, j );
				else
					break;
	...
	public void insertionSort( ArrayList<PlayerInfoVO> arr, int start, int end ) {
		...
		for ( int j = start + 1; j <= end; j++ ) {
			...
			for ( i = j; i > start && tmp.getRebRate()<arr.get(i - 1).getRebRate(); i-- ) {
